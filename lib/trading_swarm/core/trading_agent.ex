defmodule TradingSwarm.Core.TradingAgent do
  ...
  defp execute_trading_strategy(state) do
    if :rand.uniform() < state.dna.trade_frequency do
      ...
      symbol = "AAPL"
      # Simulated price
      price = 150.0 + (:rand.uniform() - 0.5) * 10
  ...
  defp execute_trade(state, trade) do
    Logger.info(
      "Agent #{state.id} executing #{trade.type} #{trade.quantity} #{trade.symbol} at $#{trade.price}"
    )
  ...
  defp calculate_fitness_update(state, _pnl_impact) do
