defmodule TradingSwarm.Core.RiskManager do
  ...
  @max_account_risk 0.15  # 15% máximo de drawdown do sistema
  @max_agent_risk 0.02    # 2% máximo por agente
  ...
  def init(_opts) do
    ...
    initial_state = %{
      ...
      drawdown_tracking: %{
        ...
        max_drawdown: 0.0,
        high_water_mark: Decimal.new(100000)  # Capital inicial
      },
      risk_metrics: initialize_risk_metrics(),
      alert_levels: %{
        warning: 0.10,   # 10% drawdown warning
        critical: 0.12,  # 12% drawdown critical
        emergency: 0.15  # 15% drawdown emergency stop
      },
  ...
  def handle_info({:evaluate_risk}, state) do
    ...
    Logger.info("Drawdown atual: #{Float.round(state.drawdown_tracking.current_drawdown * 100, 2)}%")
  ...
  defp exceeds_agent_limits?(trade, limits) do
    ...
    trade_value > (limits.max_risk || 1000.0)
  end
  ...
  defp exceeds_system_limits?(trade, state) do
    ...
    max_exposure = Decimal.mult(state.drawdown_tracking.high_water_mark, Decimal.from_float(@max_account_risk))
    Decimal.compare(new_exposure, max_exposure) == :gt
  end
  
  defp violates_correlation_limits?(_trade, _state) do
    # Implementação simplificada - em produção faria análise real de correlação
    false
  end
  
  defp calculate_agent_limits(risk_tolerance) do
    base_capital = 100000.0  # Capital base
    max_risk_amount = base_capital * risk_tolerance
    
    %{
      max_risk: max_risk_amount,
      max_position_size: max_risk_amount * 5,  # Alavancagem 5x
      daily_loss_limit: max_risk_amount * 3,
      max_trades_per_day: 100
    }
  end
  
  defp update_risk_metrics(state, agent_id, trade, pnl_impact) do
    trade_value = Decimal.from_float(trade.quantity * trade.price)
    
    new_exposure = if trade.type == :buy do
    ...
    new_agent_exposure = if trade.type == :buy do
    ...
    %{state |
      ...
      position_limits: new_position_limits
    }
  end
  
  defp update_drawdown_tracking(state, pnl_impact) do
    current_equity = Decimal.add(state.drawdown_tracking.high_water_mark, Decimal.from_float(pnl_impact))
    
    new_high_water_mark = if Decimal.compare(current_equity, state.drawdown_tracking.high_water_mark) == :gt do
      current_equity
    else
  ...
  defp broadcast_risk_alert(level, message) do
    ...
    Logger.warn("ALERTA DE RISCO [#{level}]: #{message}")
  end
  ...
  defp schedule_risk_evaluation do
    Process.send_after(self(), {:evaluate_risk}, 60_000)  # A cada minuto
  end
  
  # API Pública
  
  def validate_trades(trades, agent_limits) when is_list(trades) do
    validated = Enum.map(trades, fn trade ->
