defmodule TradingSwarm.AI.NvidiaClient do
    ...
    request_params = %{
      ...
      messages: [
        %{
          role: "system",
          content:
            "Você é um analista financeiro profissional especializado em análise de sentimento de mercado."
        },
        %{
          role: "user",
          content: prompt
    ...
    request_params = %{
      ...
      messages: [
        %{
          role: "system",
          content:
            "Você é um especialista em trading quantitativo especializado em estratégias algorítmicas."
        },
        %{
          role: "user",
          content: prompt
  ...
  defp make_api_request(params) do
    ...
    case HTTPoison.post(@base_url <> @chat_completions_endpoint, body, headers, timeout: 30_000) do
      ...
      {:ok, %HTTPoison.Response{status_code: 429}} ->
        Logger.warning("Limite de taxa da API NVIDIA excedido")
  ...
  defp parse_completion_response(response) do
    Logger.warning("Formato de resposta inesperado da API NVIDIA: #{inspect(response)}")
