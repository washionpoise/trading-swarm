defmodule TradingSwarmWeb.ChartComponents do
  ...
  def performance_line_chart(assigns) do
    chart = 
      ...
      |> LinePlot.new()
      |> LinePlot.set_val_col_names(["value"])
      |> Plot.new()
      |> Plot.titles(assigns.title, assigns.x_label, assigns.y_label)
      |> Plot.size(assigns.width, assigns.height)
      |> Plot.to_svg()
  ...
  def agent_performance_bar_chart(assigns) do
    chart = 
      ...
      |> BarChart.new()
      |> BarChart.set_val_col_names(["performance"])
      |> Plot.new()
      |> Plot.titles(assigns.title, "Agents", "P&L")
      |> Plot.size(assigns.width, assigns.height)
      |> Plot.to_svg()
  ...
  def risk_gauge(assigns) do
    ...
    angle = assigns.risk_level * 180 - 90  # -90 to 90 degrees
    ...
    color = cond do
      assigns.risk_level < 0.3 -> "#10B981"  # green
      assigns.risk_level < 0.7 -> "#F59E0B"  # yellow
      true -> "#EF4444"  # red
    end
