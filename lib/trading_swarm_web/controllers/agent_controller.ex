defmodule TradingSwarmWeb.AgentController do
  ...
  defp build_agents_query(status_filter, sort_by) do
    ...
    # Apply sorting
    case sort_by do
      "name" -> from(a in query, order_by: [asc: a.name])
      "status" -> from(a in query, order_by: [asc: a.status, asc: a.name])
      "balance" -> from(a in query, order_by: [desc: a.balance, asc: a.name])
      "trades" -> from(a in query, order_by: [desc: a.total_trades, asc: a.name])
      "win_rate" -> from(a in query, order_by: [desc: fragment("CASE WHEN ? = 0 THEN 0 ELSE ?::float / ? END", a.total_trades, a.winning_trades, a.total_trades), asc: a.name])
      "created" -> from(a in query, order_by: [desc: a.inserted_at, asc: a.name])
      _ -> from(a in query, order_by: [asc: a.name])
    end
  end
