defmodule TradingSwarmWeb.RiskController do
  ...
  defp get_risk_limits_status() do
    ...
    # For now, returning mock data
    %{
      max_exposure: %{limit: Decimal.new("10000.00"), current: Decimal.new("0.00"), utilization: 0.0},
      max_position_size: %{limit: Decimal.new("1000.00"), current: Decimal.new("0.00"), utilization: 0.0},
  ...
  defp build_risk_events_query(severity_filter, event_type_filter, resolved_filter) do
    ...
    query = if event_type_filter && event_type_filter != "" do
  ...
  defp get_risk_events_filter_options() do
    ...
    %{
      ...
      event_types: ["drawdown_warning", "position_limit_exceeded", "correlation_violation", "emergency_stop", "system_error"]
    }
  end
